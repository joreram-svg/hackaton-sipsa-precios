...
WITH ranked AS (
  ...
  SELECT producto_id, ciudad,
    max(fecha) FILTER (WHERE rn = 1) AS fecha,
    max(precio) FILTER (WHERE rn = 1) AS precio_actual,
    max(precio) FILTER (WHERE rn = 2) AS precio_1w,
    max(precio) FILTER (WHERE rn = 5) AS precio_4w,
    max(precio) FILTER (WHERE rn = 53) AS precio_52w
  FROM ranked
...
JOIN fact_precio h ON h.producto_id = c.producto_id AND h.ciudad = c.ciudad
  AND h.fecha BETWEEN c.fecha - INTERVAL '103 weeks' AND c.fecha
...
SELECT producto_id, ciudad,
  100 * regr_slope(precio, extract(epoch FROM fecha::timestamp) / 604800.0)
    / nullif(avg(precio), 0) AS pendiente_pct_sem
FROM recent
GROUP BY producto_id, ciudad;
